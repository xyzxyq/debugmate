...
For nontrivial work in this repository, first load the reusable machine profile at:

`C:\Users\20795\Documents\codex 第一次进化\docs\CODEX_EXPERIENCE_PROFILE.md`

Treat `.planning/PROJECT.md`, `.planning/REQUIREMENTS.md`, `.planning/ROADMAP.md`, and `.planning/STATE.md` as the current project source of truth. Preserve the planning/implementation boundary: roadmap creation does not authorize product implementation.
...
**Core Value:** 对一个真实或可复现的 AI/Python 工程报错，DebugMate 必须基于专属知识库给出有依据、可执行、能说明不确定性的诊断，并同步生成文字、图像和语音三种关联输出。
