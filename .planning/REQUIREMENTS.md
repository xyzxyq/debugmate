...
- [x] **MULTI-01**: 系统可以从已校验的 `DiagnosisRecord` 生成结构化中文文字诊断报告，并保留英文错误原文和命令。
- [x] **MULTI-02**: 系统可以从同一 `DiagnosisRecord` 确定性生成包含根因、检查路径和修复验证步骤的 PNG 诊断卡或流程图。
- [x] **MULTI-03**: 系统可以从同一 `DiagnosisRecord` 生成30–60秒中文复盘稿并转换为可播放、可下载的 MP3。
- [x] **MULTI-04**: 文字、PNG和MP3共享相同 `case_id`、诊断摘要哈希、Schema版本和生成版本；一致性检查失败时不得进入交付证据。
- [x] **MULTI-05**: TTS或PNG主后端不可用时，系统可以启用已记录的本地降级后端并在结果中明确标识。
...
*Last updated: 2026-07-19 after V0.1 course package verification*
