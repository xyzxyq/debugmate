...
KEY = b"k" * 32
ALL_STAGES = [
    "input_approved", "extracted", "facts_confirmed", "provisional_routed",
    "sufficiency_checked", "final_routed", "retrieved", "generated",
    "validated", "published",
]


def _approved(case_id: str, *, key: bytes = KEY, when: datetime | None = None,
              screenshot: Path | None = None, root: Path | None = None):
    ...
    preview = PreviewBundle(
        case_id=case_id, redacted=redacted, candidates=[],
        audit=RedactionAudit(candidate_count=0, counts_by_kind={}),
        source_hash="1" * 64, preview_hash="2" * 64, rule_version="test",
...
def _record(case_id: str, values: dict[str, str]) -> ExtractionRecord:
    ...
    for raw_field, value in values.items():
        ...
        candidates.append(make_candidate(field_id=field_id, value=value,
                                         source_kind=SourceKind.TEXT,
                                         confidence=1.0, locator=locator))
        cursor += len(value) + 1
    candidates.sort(key=lambda item: item.candidate_id)
    hashes = {"error_text": sha256_bytes(b"fixture")}
    return ExtractionRecord(case_id=case_id,
        extraction_id=extraction_id_for(case_id, hashes, candidates),
        source_hashes=hashes, candidates=candidates)
...
class RetrievalSpy:
    ...
    def retrieve(self, facts, routing):
        ...
        return [EvidenceAnchor(
            evidence_id="evidence_" + "4" * 32, chunk_id="fixture:1",
            content_summary="Official fictional fixture guidance.", source_id="python-docs",
            source_url="https://docs.python.org/3/", locator="fixture",
            relevance_score=0.9, knowledge_build_id=self.knowledge_build_id,
        )]
...
class GeneratorSpy:
    ...
    def generate(self, request):
        ...
        if self.fail:
            return GenerationFailed(issues=[GenerationIssue(code=IssueCode.SCHEMA_INVALID,
                pointer="/recap_text")], generation_attempts=2,
                completed_stages=["candidate_received", "local_validation"],
                run_ids=["fixture:1", "fixture:2"])
        facts = [ObservedFact(fact_id=f.fact_id, field_id=f.field_id.value, value=f.value,
            source_kind=f.source_kinds[0].value, confidence=f.confidence,
            locator=f"fact:{f.fact_id}") for f in request.observed_facts]
        # request facts are already ObservedFact; retain the public values exactly.
        facts = request.observed_facts
        payload = json.loads(Path("fixtures/cases/module_not_found/diagnosis.json").read_text(encoding="utf-8"))
        payload.update(case_id=request.case_id, category=request.routing.category.value,
                       observed_facts=[f.model_dump(mode="json") for f in facts],
                       evidence=[e.model_dump(mode="json") for e in request.evidence],
                       support_links=[], root_cause_candidates=[])
        return type("Completed", (), {"status":"completed", "diagnosis":DiagnosisRecord.model_validate(payload),
            "generation_attempts":1, "run_ids":["fixture:1"]})()
...
def _workflow(row: dict[str, object], root: Path, *, key: bytes | None = KEY):
    ...
    workflow = DiagnosisWorkflow(extraction_provider=extraction, retrieval_provider=retrieval,
        generator=generator, approval_key=key, redacted_root=root)
    return workflow, extraction, retrieval, generator


def _rows() -> list[dict[str, object]]:
    matrix = json.loads(Path("tests/fixtures/diagnosis/workflow_cases.json").read_text(encoding="utf-8"))
    assert matrix["schema_version"] == "1.0.0"
    primary = json.loads(Path("fixtures/cases/module_not_found/candidates.json").read_text(encoding="utf-8"))
...
def test_all_seven_routes_have_exactly_one_primary_matrix_case() -> None:
    completed_route_rows = [row for row in _rows() if row["case_key"] in {
        "module_not_found", "path_permission", "python_runtime", "tensor_shape_dtype",
        "cuda_memory", "model_loading", "unknown"}]
    assert [row["expected_category"] for row in completed_route_rows] == [
        "dependency_environment", "path_permission", "python_runtime",
        "tensor_shape_dtype", "cuda_memory", "model_loading", "unknown"]
...
def test_untrusted_approval_calls_no_provider(failure: str, tmp_path: Path) -> None:
    ...
    approved = _approved(str(row["case_id"]), key=b"w" * 32 if failure == "wrong_key" else KEY,
                         when=when)
    if failure == "forged":
...
def test_screenshot_failure_calls_no_provider(failure: str, tmp_path: Path) -> None:
    ...
    if failure == "missing": screenshot.unlink()
    else: screenshot.write_bytes(b"changed")
    workflow, extraction, retrieval, generator = _workflow(_rows()[0], tmp_path)
    with pytest.raises(ApprovalInvalid): workflow.run(approved)
    assert not extraction.calls and not retrieval.calls and not generator.calls


def test_unsafe_screenshot_path_calls_no_provider(tmp_path: Path) -> None:
    approved = _approved(_rows()[0]["case_id"])
    unsafe = RedactedFields.model_construct(error_text="fixture", code=None, environment={},
        redacted_screenshot_path="../escape.png", redacted_screenshot_sha256="1" * 64)
    preview = PreviewBundle.model_construct(case_id=approved.case_id, redacted=unsafe,
        candidates=[], audit=RedactionAudit(candidate_count=0, counts_by_kind={}),
        source_hash="1"*64, preview_hash="2"*64, rule_version="test",
        created_at_utc=datetime.now(UTC))
...
def test_correction_rerun_is_immutable_and_revision_bound(tmp_path: Path) -> None:
    ...
    overlay = CorrectionOverlay(case_id=original.case_id, base_revision=original.revision,
        base_facts_sha256=original.facts_sha256, field_id=target.field_id,
        fact_id=target.fact_id, old_value_sha256=sha256_bytes(target.value.encode()),
        replacement="AttributeError", reason="confirmed from redacted traceback")
